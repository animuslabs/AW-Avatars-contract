...
namespace atomicassets {
    ...
    EOSIO_REFLECT(offers_s, offer_id, sender, recipient, sender_asset_ids, recipient_asset_ids, memo, ram_payer)

    typedef multi_index<name("offers"),
                        offers_s,
    ...
    vector<uint8_t> serialize_attribute(const string& type, const ATOMIC_ATTRIBUTE& attr)
    {
        ...
        if (type == "int8") {
            ...
            check(std::holds_alternative<uint8_t>(attr), "Expected a bool (needs to be provided as uint8_t because of C++ restrictions), but got something else");
    ...
    vector<uint8_t> serialize(ATTRIBUTE_MAP attr_map, const vector<FORMAT>& format_lines)
    {
        ...
        if (attr_map.begin() != attr_map.end()) {
            check(false, "The following attribute could not be serialized, because it is not specified in the provided format: " + attr_map.begin()->first);
        }
        return serialized_data;
    }
